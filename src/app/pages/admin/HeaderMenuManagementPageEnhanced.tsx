/**
 * Header Menu Management Page - Enhanced Version (Admin Backoffice)
 * 
 * Complete interface for managing header menu configuration and items
 * Features: Drag & Drop, Create/Edit forms, Icon selector, Statistics
 * 
 * Based on FooterMenuManagementPageEnhanced but adapted for Header
 */
...
import {
  ...
  useHeaderMenuStats,
} from '../../hooks/useHeaderMenuApi';
import type { HeaderMenuItemDTO, CreateHeaderMenuItemDTO, UpdateHeaderMenuItemDTO } from '../../types';

const ItemType = 'MENU_ITEM';

export function HeaderMenuManagementPageEnhanced() {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600">
              Configure and manage the dynamic header menu displayed on the FrontOffice
            </p>
          </motion.div>
...
function MenuItemsTab() {
  const { data: menuItems, isLoading } = useHeaderMenuItems();
  const [editingItem, setEditingItem] = useState<HeaderMenuItemDTO | null>(null);
  const [isCreating, setIsCreating] = useState(false);
  const batchUpdateOrder = useBatchUpdateHeaderMenuOrder();
...
interface DraggableMenuItemProps {
  item: HeaderMenuItemDTO;
  onDrop: (draggedId: string, targetId: string) => void;
  onEdit: () => void;
}

function DraggableMenuItem({ item, onDrop, onEdit }: DraggableMenuItemProps) {
  const toggleActive = useToggleHeaderMenuItemActive();
  const toggleVisibility = useToggleHeaderMenuItemVisibility();
  const deleteItem = useDeleteHeaderMenuItem();
...
interface MenuItemFormModalProps {
  item: HeaderMenuItemDTO | null;
  onClose: () => void;
}

function MenuItemFormModal({ item, onClose }: MenuItemFormModalProps) {
  const isEditing = !!item;
  const createItem = useCreateHeaderMenuItem();
  const updateItem = useUpdateHeaderMenuItem();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: CreateHeaderMenuItemDTO = {
      key: formData.key,
      label: {
        fr: formData.labelFr,
        de: formData.labelDe,
              ...
              <div className="flex items-center gap-6 pt-6">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
                  />
                  <span className="text-sm font-medium text-gray-700">Show in Header</span>
                </label>
              </div>
            </div>
          </div>
...
function ConfigurationTab() {
  const { data: config, isLoading } = useHeaderMenuConfig();
  const updateConfig = useUpdateHeaderMenuConfig();

  const [formData, setFormData] = useState({
    logoVisible: config?.logo.isVisible ?? true,
    logoWidth: config?.logo.width ?? 40,
    logoHeight: config?.logo.height ?? 40,
    logoPosition: config?.layout.position ?? 'left',
    ...
    stickyHeader: config?.behavior.stickyHeader ?? true,
  });
  ...
  const handleSave = () => {
    updateConfig.mutate({
      ...
      layout: {
        position: formData.logoPosition as 'left' | 'center' | 'right',
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-900">Header Menu Configuration</h2>
        <button
          onClick={handleSave}
          ...
          <div className="col-span-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
              />
              <span className="text-sm font-medium text-gray-700">Show Logo in Header</span>
            </label>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <div>
              ...
              <span className="text-xs text-gray-500">Stick to top on scroll</span>
            </div>
          </label>
        </div>
      </div>
...
function StatisticsTab() {
  const { data: stats, isLoading } = useHeaderMenuStats();
